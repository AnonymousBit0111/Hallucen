...
const std::set<std::string> &VK::getRequestedLayerNames() {
...
std::vector<std::string> VK::getAvailableVulkanExtensions(SDL_Window *window) {
  ...
  for (unsigned int i = 0; i < ext_count; i++) {
    ...
    exts.emplace_back(ext_names[i]);
  }
#ifdef DEBUG

  exts.emplace_back(VK_EXT_DEBUG_REPORT_EXTENSION_NAME);
#endif // DEBUG
  std::cout << "\n";
...
std::vector<std::string> VK::getAvailableVulkanLayers() {
...
VkInstance VK::createInstance(std::vector<std::string> layers,
  ...
  app_info.pNext = NULL;
  app_info.pApplicationName = "Hallucen";
  app_info.applicationVersion = 1;
  app_info.pEngineName = "Hallucen Engine";
  ...
  case VK_ERROR_INCOMPATIBLE_DRIVER:
    std::cout << "unable to create vulkan instance, cannot find a compatible "
                 "Vulkan ICD\n";
    exit(-1);
  default:
...
VkResult VK::createDebugReportCallbackEXT(
...
VkDebugReportCallbackEXT VK::setupDebugCallback(VkInstance &instance) {
  ...
  if (VK::createDebugReportCallbackEXT(instance, &createInfo, nullptr,
                                       &callback) != VK_SUCCESS) {
...
VkPhysicalDevice VK::selectGPU(unsigned int &graphicsQueueindex,
                               VkInstance instance) {
  ...
  for (auto &physical_device : physical_devices) {
    vkGetPhysicalDeviceProperties(physical_device,
                                  &(physical_device_properties[count]));
  ...
  if (physical_device_count > 1) {
    while (true) {
      ...
      if (selection_id >= physical_device_count || selection_id < 0) {
        std::cout << "invalid selection, expected a value between 0 and "
  ...
  std::cout << "selected: "
            << physical_device_properties[selection_id].deviceName << "\n";
  VkPhysicalDevice selected_device = physical_devices[selection_id];

  // Find the number queues this device supports, we want to make sure that we
  // have a queue that supports graphics commands
  unsigned int family_queue_count(0);
  vkGetPhysicalDeviceQueueFamilyProperties(selected_device, &family_queue_count,
                                           nullptr);
  ...
  if (queue_node_index < 0) {
    std::cout << "Unable to find a queue command family that accepts graphics "
                 "commands\n";
    exit(-1);
  }
  ...
  graphicsQueueindex = queue_node_index;

  return selected_device;
}

bool VK::createLogicalDevice(VkPhysicalDevice &physicalDevice,
                             ...
                             VkDevice &outDevice) {
  {
    ...
    if (vkEnumerateDeviceExtensionProperties(
            physicalDevice, NULL, &device_property_count, NULL) != VK_SUCCESS) {
    ...
    if (vkEnumerateDeviceExtensionProperties(
            physicalDevice, NULL, &device_property_count,
            device_properties.data()) != VK_SUCCESS) {
    ...
    const std::set<std::string> &required_extension_names =
        getRequestedDeviceExtensionNames();
    int count = 0;
    for (const auto &ext_property : device_properties) {
      std::cout << count << ": " << ext_property.extensionName << "\n";
      auto it = required_extension_names.find(
          std::string(ext_property.extensionName));
    ...
    create_info.enabledExtensionCount =
        static_cast<uint32_t>(device_property_names.size());
    ...
    VkResult res =
        vkCreateDevice(physicalDevice, &create_info, nullptr, &outDevice);
    if (res != VK_SUCCESS) {
...
bool VK::createLogicalDevice(VkPhysicalDevice &physicalDevice,
                             ...
                             const std::vector<std::string> &layerNames,
                             VkDevice &outDevice);
bool VK::createSurface(SDL_Window *window, VkInstance instance,
  ...
  vkGetPhysicalDeviceSurfaceSupportKHR(gpu, graphicsFamilyQueueIndex,
                                       outSurface, &supported);
  if (!supported) {
...
bool VK::getPresentationMode(VkSurfaceKHR surface, VkPhysicalDevice device,
                             VkPresentModeKHR &ioMode) {
  uint32_t mode_count(0);
  if (vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &mode_count,
                                                NULL) != VK_SUCCESS) {
  ...
  if (vkGetPhysicalDeviceSurfacePresentModesKHR(
          device, surface, &mode_count, available_modes.data()) != VK_SUCCESS) {
    std::cout
        << "unable to query the various present modes for physical device\n";
    return false;
  }
...
bool VK::getSurfaceProperties(VkPhysicalDevice device, VkSurfaceKHR surface,
...
VkExtent2D VK::getSwapImageSize(const VkSurfaceCapabilitiesKHR &capabilities) {
  // Default size = window size
  VkExtent2D size = {(unsigned int)1280, (unsigned int)720};

  // This happens when the window scales based on the size of an image
  if (capabilities.currentExtent.width == 0xFFFFFFF) {
    size.width =
        glm::clamp<unsigned int>(size.width, capabilities.minImageExtent.width,
                                 capabilities.maxImageExtent.width);
    size.height = glm::clamp<unsigned int>(size.height,
...
bool VK::getImageUsage(const VkSurfaceCapabilitiesKHR &capabilities,
                   VkImageUsageFlags &outUsage) {
  const std::vector<VkImageUsageFlags> &desir_usages =
      getRequestedImageUsages();
  ...
  for (const auto &desired_usage : desir_usages) {
    VkImageUsageFlags image_usage =
        desired_usage & capabilities.supportedUsageFlags;
...
bool VK::getFormat(VkPhysicalDevice device, VkSurfaceKHR surface,
  ...
  if (vkGetPhysicalDeviceSurfaceFormatsKHR(
          device, surface, &count, found_formats.data()) != VK_SUCCESS) {
  ...
  if (found_formats.size() == 1 &&
      found_formats[0].format == VK_FORMAT_UNDEFINED) {
  ...
  for (const auto &found_format_outer : found_formats) {
    ...
    if (found_format_outer.format == gFormat) {
      ...
      std::cout << "warning: no matching color space found, picking first "
                   "available one\n!";
  ...
  std::cout << "warning: no matching color format found, picking first "
               "available one\n";
...
bool VK::createSwapChain(VkSurfaceKHR surface, VkPhysicalDevice physicalDevice,
  ...
  // Create new one
  if (vkCreateSwapchainKHR(device, &swap_info, nullptr, &old_swap_chain) !=
      VK_SUCCESS) {
...
bool VK::getSwapChainImageHandles(VkDevice device, VkSwapchainKHR chain,
  ...
  if (vkGetSwapchainImagesKHR(device, chain, &image_count,
                              outImageHandles.data()) != VK_SUCCESS) {
